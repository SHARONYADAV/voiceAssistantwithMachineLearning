...
chrome_path = 'C:\\Users\\Sharon\\AppData\\Local\\Google\\Chrome\\Application\\chrome.exe'
...
def hello():
    ...
    AUDIO("I am your Voice Assistant. Please tell me how may I help you")       
...
def userinput():
    ...
    try:
        ...
        # print(e)    
        AUDIO("I'm sorry, i don't understand")
        return "None"
    return query
...
#find_file_name('D:\\','Alexa')   
#*************************************************************************************************

def scommand():  #for assistant to ask the user
    r = sr.Recognizer()
    with sr.Microphone() as source:
...
#*************************************************************************************************
def get_category():
    AUDIO('What pictures do you want me to show?')
    AUDIO('Plese tell me the category')
    r = scommand()
    categories = [r]  #return category
    AUDIO(f"Searching {categories} related images") 
    return categories
    ...
    try:
        for i in category:
            ...
            if i=='cat' or i=='dog':
                ...
                model = pickle.load(open('model\\transportationneww.p','rb'))
                categories= ['car','plane','cycle']
            elif i=='apple' or i=='grapes' or i == 'kiwi' or i=='papaya':
                model = pickle.load(open('model\\fruitsneww.p','rb'))
                categories= ['apple','grapes','kiwi','papaya']
            else:
                break
            #******************************************************Testing
            AUDIO('  Testing started...')
            ...
            f =[]
            dest = "D:/AARUSH/resultimage" #where u want to copy
            path = "D:/AARUSH/predictions" #for predicting data
            for dir, subdir, files in walk(path):
                ...
                for i in f:
                    ...
                    #testing brand new image
                        ...
                        #url = input('Enter url')
                        url = path+'/'+base #getting file name and exc  (file name+path=absolute path)
                        print(url)
                        img = imread(url)
                        ...
                        y_out = categories[y_out[0]]# categorize
                        # print(type(y_out)) #array
                        # print(type(category[0]))#string
                        print(f'predicted output : {y_out}')   #this displays the type of image
                        print("-"*100)
                        if y_out == category[0]: #it change in string type.category 0 and catagories then its same
                            copyfile(url,(dest+'/'+base))                        
        os.startfile(dest) #open file
    except Exception as e:
        print(e)
